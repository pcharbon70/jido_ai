defmodule Jido.AI.ReqLlmBridge.ToolIntegrationManagerTest do
  ...
  alias Jido.AI.ReqLlmBridge.{ToolIntegrationManager, ToolBuilder, ToolResponseHandler, ConversationManager}
  ...
  describe "generate_with_tools/3" do
    test "successfully generates response with tool execution", %{weather_action: action} do
      ...
      assert {:ok, response} = ToolIntegrationManager.generate_with_tools(
  ...
  describe "conversation management" do
    ...
    test "continue_conversation/3 processes message with context", %{weather_action: action} do
      ...
      expect(ToolResponseHandler, :process_llm_response, fn _response, ^conversation_id, _options ->
        {:ok, processed_response}
      end)

      expect(ConversationManager, :add_assistant_response, fn ^conversation_id, ^processed_response ->
        :ok
      end)

      assert {:ok, response} = ToolIntegrationManager.continue_conversation(
        conversation_id,
        message
      )
  ...
  describe "streaming support" do
    test "handles streaming requests", %{weather_action: action} do
      ...
      expect(ToolResponseHandler, :process_streaming_response, fn ^stream, "conv_stream", _options ->
        {:ok, processed_response}
      end)
  ...
  describe "tool choice parameter handling" do
    test "maps tool choice parameters correctly", %{weather_action: action} do
      ...
      Enum.each(test_cases, fn {input_choice, expected_output} ->
        ...
        assert {:ok, _response} = ToolIntegrationManager.generate_with_tools(
          "Test message",
          [action],
          options
        )
  ...
  describe "error handling and edge cases" do
    ...
    test "handles response processing errors gracefully", %{weather_action: action} do
      ...
      assert {:error, "Processing failed"} = ToolIntegrationManager.generate_with_tools(
        "Test",
        [action]
      )
    end
