defmodule Jido.AI.Runner.GEPA.Selection.SelectionIntegrationTest do
  ...
  describe "complete NSGA-II selection pipeline" do
    test "environmental selection + elite preservation + tournament selection" do
      ...
      {:ok, tournament_winners} = TournamentSelector.select(
        survivors,
        count: 25,
        strategy: :pareto,
        tournament_size: 3
    ...
    test "full generation cycle with all selection mechanisms" do
      ...
      final_population = Enum.reduce(1..5, population, fn gen, current_pop ->
  ...
  describe "fitness sharing integration with selection" do
    test "fitness sharing + tournament selection promotes diversity" do
      ...
      {:ok, without_sharing} = TournamentSelector.select(
        population,
        count: 25,
        strategy: :pareto,
        tournament_size: 3
      ...
      {:ok, shared_population} = FitnessSharing.apply_sharing(
        population,
        niche_radius: 0.2
      )

      {:ok, with_sharing} = TournamentSelector.select(
        shared_population,
        count: 25,
        strategy: :pareto,
        tournament_size: 3
    ...
    test "adaptive fitness sharing activates when diversity drops" do
      ...
      result_diverse = FitnessSharing.adaptive_apply_sharing(
        diverse_pop,
      ...
      result_clustered = FitnessSharing.adaptive_apply_sharing(
        clustered_pop,
        diversity_threshold: 0.3,
        diversity_metric: :crowding
      )
  ...
  describe "elite preservation integration scenarios" do
    test "frontier-preserving elite selection + environmental selection" do
      ...
      ] ++ create_diverse_population(43, generation: 1)  # Fill to 50
    ...
    test "diversity-preserving elite selection prevents duplicates" do
      ...
      unique_in_elites = Enum.count(diverse_elites, fn c ->
  ...
  describe "tournament selection strategy integration" do
    test "adaptive tournament adjusts to population diversity state" do
      ...
      {:ok, selected_high} = TournamentSelector.select(
        high_diversity,
        count: 25,
        strategy: :adaptive
      )
      ...
      {:ok, selected_low} = TournamentSelector.select(
        low_diversity,
        count: 25,
        strategy: :adaptive
      )
    ...
    test "diversity-aware tournament vs pareto tournament trade-offs" do
      ...
      population = [
        ...
        create_candidate("hf_1", fitness: 10.0, normalized_objectives: %{accuracy: 0.9, latency: 0.1}),
        create_candidate("hf_2", fitness: 9.5, normalized_objectives: %{accuracy: 0.91, latency: 0.11}),
        create_candidate("hf_3", fitness: 9.0, normalized_objectives: %{accuracy: 0.92, latency: 0.12}),
        # Lower fitness, high diversity (isolated)
        create_candidate("hd_1", fitness: 7.0, normalized_objectives: %{accuracy: 0.3, latency: 0.7}),
        create_candidate("hd_2", fitness: 6.5, normalized_objectives: %{accuracy: 0.1, latency: 0.9})
      ] ++ create_diverse_population(45, generation: 1)

      population = assign_pareto_metrics(population)

      # Pareto strategy: prioritizes fitness (rank, then distance)
      {:ok, pareto_selected} = TournamentSelector.select(
        population,
        count: 25,
        strategy: :pareto,
        tournament_size: 3
      ...
      {:ok, diversity_selected} = TournamentSelector.select(
        population,
        count: 25,
        strategy: :diversity,
        tournament_size: 3
  ...
  describe "realistic GEPA optimization scenarios" do
    test "prompt optimization with multiple objectives" do
      ...
      evolution_history = Enum.reduce(1..3, {initial_population, []}, fn gen, {current_pop, history} ->
        ...
        selection_pop = case FitnessSharing.adaptive_apply_sharing(
          survivors,
          diversity_threshold: 0.3,
          niche_radius: 0.1
        ) do
      ...
      assert gen1.elite_count == 10  # 20% of 50
    ...
    test "converged population maintains diversity through selection" do
      ...
      assert length(diverse_elites) <= 20  # May be less if too many duplicates filtered
    ...
    test "boundary solution preservation through complete pipeline" do
      ...
      population = [
        ...
        create_candidate("boundary_max_acc", normalized_objectives: %{accuracy: 1.0, latency: 0.5}),
        # Boundary: min accuracy
        create_candidate("boundary_min_acc", normalized_objectives: %{accuracy: 0.0, latency: 0.5}),
        # Boundary: max latency
        create_candidate("boundary_max_lat", normalized_objectives: %{accuracy: 0.5, latency: 1.0}),
        # Boundary: min latency
        create_candidate("boundary_min_lat", normalized_objectives: %{accuracy: 0.5, latency: 0.0})
  ...
  describe "performance and scalability" do
    test "handles large population efficiently" do
      ...
      {:ok, _parents} = TournamentSelector.select(
        survivors,
        count: 100,
        strategy: :adaptive,
        tournament_size: 3
    ...
    test "handles degenerate cases gracefully" do
      ...
      identical_pop = Enum.map(1..50, fn i ->
  ...
  defp assign_pareto_metrics(population) do
    ...
    population = Enum.map(population, fn c ->
