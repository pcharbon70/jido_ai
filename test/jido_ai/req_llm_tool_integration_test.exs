defmodule Jido.AI.ReqLLMToolIntegrationTest do
  ...
  alias Jido.AI.ReqLLM.ToolBuilder
  ...
  describe "end-to-end tool integration" do
    test "converts Action to ReqLLM tool and executes successfully", %{weather_action: action} do
      ...
      assert {:ok, [tool]} = ReqLLM.convert_tools([action])
    ...
    test "handles tool choice parameters in request options", %{weather_action: weather, calculator_action: calc} do
      ...
      Enum.each(test_cases, fn {input_choice, expected_output} ->
        ...
        options = ReqLLM.build_req_llm_options(params)
    ...
    test "batch converts multiple actions with mixed success", %{weather_action: valid} do
      ...
      assert {:ok, tools} = ReqLLM.convert_tools(actions)
    ...
    test "validates action compatibility before conversion", %{weather_action: action} do
      ...
      assert :ok = ReqLLM.validate_tool_compatibility(action)
      ...
      assert {:error, error} = ReqLLM.validate_tool_compatibility(BrokenAction)
  ...
  describe "tool execution error handling" do
    ...
    test "handles timeout scenarios" do
      ...
      options = %{timeout: 50}
      assert {:ok, [tool]} = ReqLLM.convert_tools_with_options([SlowAction], options)
  ...
  describe "advanced tool features" do
    ...
    test "supports tool execution with context", %{weather_action: action} do
      ...
      assert {:ok, [tool]} = ReqLLM.convert_tools_with_options([action], options)
  ...
  describe "backward compatibility" do
    test "maintains compatibility with existing tool consumers" do
      ...
      assert {:ok, tools} = ReqLLM.convert_tools([WeatherAction])
      assert length(tools) == 1
    ...
    test "preserves error format compatibility" do
      ...
      assert {:error, error} = ReqLLM.convert_tools([WeatherAction])
  ...
  describe "performance and scalability" do
    ...
    test "handles concurrent tool execution safely" do
      ...
      assert {:ok, [tool]} = ReqLLM.convert_tools([WeatherAction])
