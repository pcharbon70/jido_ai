...
kind: package
status: active
summary: jido_ai is the Jido ecosystem AI runtime layer for tool-using agents, reusable actions, direct LLM facade calls, and explicit reasoning strategy execution through ReqLLM-backed provider abstraction.
decisions:
...
- id: package.jido_ai.explicit_policy_boundaries
  statement: jido_ai shall keep model routing, timeout, retry, structured I/O, and tool policy explicit while keeping provider-specific behavior behind ReqLLM integration boundaries.
  priority: must
  stability: stable

- id: package.jido_ai.spec_led_workspace
  statement: The repository shall maintain a package-local .spec workspace for current-truth specs, durable ADRs, and generated spec state aligned with code, guides, and tests.
  priority: must
  stability: evolving
```
